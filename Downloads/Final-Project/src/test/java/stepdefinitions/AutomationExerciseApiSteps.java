...
public class AutomationExerciseApiSteps {
    ...
    public void validateResponseCode(int expectedCode) {
        ...
        // If the response contains a JSON "responseCode" field, validate it too
        if (response.getContentType().contains("application/json") &&
                response.jsonPath().get("responseCode") != null) {
            int responseCode = response.jsonPath().getInt("responseCode");
            Assert.assertEquals(responseCode, expectedCode,
                    "API responseCode field mismatch");
        }
    }
    ...
    public void validateUserDetailsResponse() {
        Assert.assertTrue(response.statusCode() == 200 || response.statusCode() == 404,
                "Unexpected HTTP status for user details API");
